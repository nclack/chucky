...
// Build lifted layout (same logic as stream_init.c compute_level_layout).
static void
                    ...
                    uint64_t* out_epoch_elements)
{
  *out_lifted_rank = (uint8_t)(2 * rank);
  ...
  uint64_t tile_stride = tile_elements; // no codec alignment

  compute_lifted_strides(
    rank, tile_sizes, tile_count, NULL, (int64_t)tile_stride, lifted_strides);
...
test_ingest_single_epoch(void)
{
  ...
  build_lifted_layout(rank, dim_sizes, tile_sizes, &lifted_rank,
...
main(int ac, char* av[])
{
  ...
  ecode |= test_ingest_single_epoch();
  ecode |= test_ingest_chunked();
  ecode |= test_ingest_multiscale();
